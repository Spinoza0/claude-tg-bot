...
- `BOT_LANG` — язык сообщений бота (Telegram + консоль): `en` или `ru`. Один
  язык = один файл в `claude_tg_bot/locale/`. Пусто/неизвестно → `en`.
- `AGENT_GENDER` — грамматический род агента (`male` | `female`), чтобы бот
  обращался к себе в правильной форме. Claude получает подсказку о роде через
  дополнительный системный промпт (пишет корректно на любом языке), а свои
  русские сообщения бот согласует по роду. Пусто → `male`.

`BOT_LANG`, `AGENT_GENDER` и `SANDBOX_COMMAND` — **живые**: их можно менять на
лету из разрешённого чата (обычным сообщением, без кодового слова песочницы)
через `/config ПАРАМЕТР=ЗНАЧЕНИЕ ...` (полные имена настроек, регистр не важен).
Без аргументов `/config` показывает их и текущие значения.
...
| `/config ПАРАМЕТР=ЗНАЧЕНИЕ ...` | Изменить настройку на лету (BOT_LANG, AGENT_GENDER, SANDBOX_COMMAND); без аргументов — показать их и текущие значения |
