...
# Не даём макбуку заснуть, пока бот работает: caffeinate держит систему
# (display/idle/disk/system) бодрствующей на всё время жизни процесса. Пока
# бот жив — ноутбук не уходит в сон (иначе теряется контекст сессии).
# Если caffeinate недоступен (не macOS) — запускаем без него.
echo "==> Запускаю бота..."
