...
- **Контекст** — бот продолжает последнюю сессию каталога через `--continue`
  (как локальный `claude --continue`). Если сессии нет — запускается новая.
  `session_id` хранится на диске и не сохраняется в state бота, поэтому
  локальный запуск в том же каталоге подхватывает ту же сессию.
