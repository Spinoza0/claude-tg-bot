...
- **Контекст** — `session_id` сохраняется на пользователя/проект, следующее
  сообщение продолжает ту же сессию через `--resume <id>`.
...
- `SANDBOX_ROOT` — каталог (песочница), где запускается агент для сообщений
...
- `AUTO_DELETE_MEDIA` — автоудалять ли вложение после отправки в Claude.
  ...
  вручную командой `/clearmedia`.
- `DELETE_MODE` — куда удалять: `trash` (в корзину macOS, по умолчанию) или
  `permanent` (навсегда). Работает и для `/clearmedia`, и при автоудалении.
...
| `/clean` | Сбросить контекст (начать новую сессию) |
...
## Запуск агента в любом чате (`@helpbot`)

Помимо активного проекта, бот умеет запускать агента **в любом чате**, где он
...
- Сообщение обязано **начинаться** с упоминания `@helpbot`, дальше пробел и
  ...
  в агентском контексте (корень `SANDBOX_ROOT`, отдельный выбор проекта). Примеры:
  - `@helpbot проверить README на ошибки` → Claude в агентском каталоге
  ...
  - `@helpbot /clear` → сброс контекста в агентской сессии
  - `@helpbot` + фото/видео/файл → Claude с вложением
- Отправитель обязан быть в `ALLOWED_USERS`. Ограничение на чат
  (`ALLOWED_CHAT_IDS`) для `@helpbot` **не применяется** — агента можно вызвать
...
- **Раздельное состояние**: `@helpbot`-проекты (`SANDBOX_ROOT/...`) хранятся
  ...
  `@helpbot` меняют только агентский проект и не трогают обычный, и наоборот.
  Сессия Claude привязана к конкретному каталогу, поэтому обе ветки можно
  вести параллельно, контекст не пересекается.
- Агент запускается в каталоге `SANDBOX_ROOT` (или песочнице по умолчанию
  `~/.claude-tg-bot/sandbox`), независимо от активного проекта пользователя.
  Если агентский проект не выбран — работает прямо в корне `SANDBOX_ROOT`.
...
- Обычные проекты (`PROJECTS_ROOT`) и агентские (`SANDBOX_ROOT`) — два
