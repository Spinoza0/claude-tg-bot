...
- `COMMAND_ARGS_ALTERNATIVE` — альтернативный набор аргументов для **смены
  ...
  пусто — запрос повторяется базовыми аргументами без смены модели.
...
`config.env` и запускает бота (`python -m claude_tg_bot`). Если в `config.env`
включён `KEEP_AWAKE=true`, скрипт держит макбук бодрствующим (`caffeinate -dimsu`),
чтобы при засыпании не отключалась сеть и бот не переставал принимать/отвечать
на сообщения.
