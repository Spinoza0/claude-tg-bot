...
`config.env` и запускает бота (`python -m claude_tg_bot`). Пока бот работает,
скрипт держит макбук бодрствующим (`caffeinate -dimsu`) — ноутбук не засыпает
и не теряет контекст сессии.
